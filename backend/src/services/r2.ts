...
// Used for download flows where the browser navigates to the URL (no HEAD
// probe). NOT for <video src> — Chrome HEAD-probes cross-origin media src
// and R2 returns 503 on HEAD via a presigned URL.
...
// /media/:id route can stream video to the browser with seek support. The
// proxy path is what `<video>` elements use; the presigned-URL path above
// only works for navigations (downloads).
